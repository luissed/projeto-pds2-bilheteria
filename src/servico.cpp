...
void validarId(std::string* entrada)
{
    bool digito;
    do{
        digito = true;
        std::cin >> *entrada;
        for (unsigned long int c=0; c<entrada->size();++c){
            if (!std::isdigit(entrada->at(c))){
                digito = false;
                std::cout << "entrada deve conter apenas números. Tente novamente." << std::endl;
                break;
            }
        }
    } while (!digito);
...
void validarCPF(std::string *cpf)
{
    ...
    do{
        ...
        std::cin >> *cpf;
        if (cpf->size() == 11){
            for (unsigned long int c=0; c<cpf->size();++c){
                if (!std::isdigit(cpf->at(c))){
                    valido = false;
                    std::cout << "CPF DEVE TER APENAS NUMEROS" << std::endl;
                    break;
                }
            }
            ...
            if (soma1 % 11 == 0 || soma1 % 11 == 1|| soma1 % 11 == 10){
                if ((cpf->at(9) - 48) != 0){
                    valido = false;
                }
            }
            ...
            if (soma2 % 11 == 0 || soma2 % 11 == 1|| soma2 % 11 == 10){
                if ((cpf->at(10) - 48) != 0){
                    valido = false;
                }
            }
...
bool Servico::editarFuncionario(unsigned long int idFuncionario)
{
    if (this->_funcionarios.find(idFuncionario) != this->_funcionarios.end())
    {
        ...
        {
            std::string novoCargo;
            std::cout << "1. CAIXA\t2. ORGANIZADOR" << std::endl;
            std::cout << "3. GERENTE" << std::endl;
            validarId(&novoCargo);
            this->_funcionarios.at(idFuncionario)->alterarCargo(std::stoi(novoCargo, NULL, 10));
        }
        this->_funcionarios.at(idFuncionario)->exibirFuncionario();
        std::cout << "####################" << std::endl;
        return true;
...
bool Servico::editarSala(unsigned long int idSala)
{
    if (this->_salas.find(idSala) != this->_salas.end())
    {
        ...
        if (resp[0] == 50 || resp[0] == 52 || resp[0] == 54 || resp[0] == 55)
        {
            std::string coluna, linha;
            std::cout << "NOVA TAMANHO:" << std::endl;
            std::cout << "COLUNA:";
            validarId(&coluna);
            std::cout << "LINHA:";
            validarId(&linha);
            this->_salas.at(idSala)->editarSala(std::stoi(coluna, NULL, 10), std::stoi(linha, NULL, 10));
        }
        if (resp[0] == 51 || resp[0] == 53 || resp[0] == 54 || resp[0] == 55)
        {
            ...
            validarId(&novoValor);
            this->_salas.at(idSala)->editarSala(std::stoi(novoValor, NULL, 10));
        }
        this->_salas.at(idSala)->exibirSala();
        std::cout << "####################" << std::endl;
        return true;
...
void Servico::relatorioMensal(void)
{
    ...
    std::string mes(aux, 80);
    std::ifstream ifs("./relatorio/" + mes + ".txt");
    if(ifs.is_open()){ 
        std::ofstream outfile("./relatorio/" + mes + ".txt", std::ofstream::out);
        if (outfile.is_open())
        {
...
void Servico::adicionarCliente(std::string nome, std::string email, unsigned short int idade, std::string cpf)
{
    if(this->_clientes.find(cpf)==this->_clientes.end()){
        ...
        std::cout<<"CLIENTE JA CADASTRADO"<<std::endl;
    }
}
...
void Servico::venderIngressos(unsigned long int idFuncionario, unsigned long int idSessao, std::string nome, std::string cpf)
{
    if (this->_clientes.find(cpf) == this->_clientes.end())
    {
        ...
        if (resp[0] == 49)
        {
            std::string email, idade;
            std::cout << "EMAIL: ";
            std::cin >> email;
            std::cout << "IDADE: ";
            validarId(&idade);
            adicionarCliente(nome, email, std::stoi(idade, NULL, 10), cpf);
        }
        else
        {
